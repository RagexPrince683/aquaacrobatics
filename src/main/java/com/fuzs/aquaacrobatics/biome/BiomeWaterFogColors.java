...
public abstract class BiomeWaterFogColors {
    ...
    public static int getWaterFogColorForBiome(BiomeGenBase biome) {
        ResourceLocation location = new ResourceLocation(biome.biomeName);
        if (location == null) return DEFAULT_WATER_FOG_COLOR;
        Integer color = fogColorMap.get(location);
        if (color != null) return color;
        return DEFAULT_WATER_FOG_COLOR;
    }

    public static int getWaterColorForBiome(BiomeGenBase biome, int oldColor) {
        ResourceLocation location = new ResourceLocation(biome.biomeName);
        if (location == null) {
        ...
        if (oldColor != DEFAULT_WATER_COLOR_112) {
            AquaAcrobatics.LOGGER.info(
                "Potentially missing water color mapping for " + location + ", attempting to fake old appearance");
            color = emulateLegacyColor(oldColor);
        } else color = DEFAULT_WATER_COLOR;
